...
module.exports = {
	...
	train: function(emoji,msg) {
		...
		//dbo.collection("training_phrases").insertOne(phrase)
  ...
  classify: function(emoji) {
		...
		//console.log("USER TRAINING BAD")

		//console.log(user_training_good)
		//console.log("***")

		// create some 'good' items (name, characteristics)
		//if (good_file) { good_emojis = this.parseTrainingFile(good_file) }
		//if (emu_file) { emu_emojis = this.parseTrainingFile(emu_file) }
		//if (potential_file) { potential_emojis = this.parseTrainingFile(potential_file) }

		//console.log(this.parseTrainingData(user_trained))

		//if (user_trained) { good_emojis = user_trained }
		//if (actual_reactions) { emu_emojis = actual_reactions }
		//if (potential_reactions) { potential_emojis = potential_reactions }
		...
		if (bad_file) { bad_emojis = this.parseTrainingFile(bad_file) }
		...
		for (var i = 0; i < emoji_indirect_match.length; i++) {
			...
			for (var j = 0; j < keywords_direct_match.length; j++) {
				...
				if (keywords_direct_match[j] == keywords_indirect_match[i]) {
					if (direct_classified.probability > indirect_classified.probability) {
						// filter out equal matches by probability
						//console.log("Do not add. Duplicate Emoji")
						do_not_add = true
					}
				}
			}
  ...
  randomTreatEmoji: function() {
	...
	load_training_data: async function() {
     ...
     // db.collection('your_collection').find(...)
			...
			//var dictionary_db = db.db("emuji")
			//const collection_good = client.db("emuji").collection("user_training_good")
			...
			stream.on("end", function() {
				//saveTrainingData(user_training_good,user_training_bad)
...
function saveTrainingData(good,bad) {
	//console.log(user_training_good)
	for (i in good) {
		var emoji = good[i]
		console.log(emoji)
